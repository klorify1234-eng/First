...
function renderTimeline() {
  ...
  if (sorted.length === 0) {
    timelineList.innerHTML = `<p class="timeline-empty">No entries yet. Use "Add entry" to log the first dated event.</p>`;
    return;
  }
...
document.getElementById("add-entry-btn").addEventListener("click", () => {
  const date = prompt("Date of this event (YYYY-MM-DD):");
  if (!date) return;
  const title = prompt("Short title for this entry:") || "Untitled entry";
  const body = prompt("Describe what happened, factually:") || "";
  const tagsRaw = prompt("Tags (comma separated — e.g. platform, evidence #), optional:") || "";
  CASE_DATA.entries.push({
    date,
    title,
    body,
